...
function syncHumanNodesToVault(opts: GenerateOptions, result: GenerateResult): void {
  ...
  while (true) {
    ...
    // R40 (M4): bulk-fetch ALL code nodes referenced by this page in ONE call
    // (chunked internally) instead of N× getNodesByIds in renderAutoGeneratedSection.
    // For a page of 500 notes each linking ~3 code symbols, this is 1 query
    // instead of 500 — a 500× reduction in SQL round-trips during export.
    let preloadedCodeNodes: Map<number, CodeNode> | undefined;
    if (opts.codeReader) {
      const allCbmIds = new Set<number>();
      for (const n of page) {
        for (const id of n.cbm_node_ids) allCbmIds.add(id);
      }
      if (allCbmIds.size > 0) {
        preloadedCodeNodes = opts.codeReader.getNodesByIds([...allCbmIds]);
      }
    }
 ...
 */
function syncSingleNode(node: HumanNode, opts: GenerateOptions, result: GenerateResult, preloadedCodeNodes?: Map<number, CodeNode>): void {
  ...
  if (existingContent === null) {
    // Create new file.
    const newContent = renderNoteForVault(node, opts.codeReader, preloadedCodeNodes);
    if (!opts.dryRun) {
    ...
    const fullContent = rebuildNoteContent(existingContent, node, opts.codeReader, preloadedCodeNodes);
 ...
 */
function rebuildNoteContent(existingContent: string, node: HumanNode, codeReader?: CodeGraphReader, preloadedCodeNodes?: Map<number, CodeNode>): string {
  const parsed = parseNote(existingContent);
  const sections = splitSections(parsed.body);
  const newAutoGenerated = renderAutoGeneratedSection(node, codeReader, preloadedCodeNodes);
...
function autoGenerateModuleNotes(opts: GenerateOptions, result: GenerateResult): void {
  ...
  // R47 (H2): bulk-fetch neighbors for ALL modules in 3 queries (was N×2).
  // getBulkNeighbors returns the same shape as getNeighbors, so the render
  // function only needs the pre-fetched array instead of calling getNeighbors
  // per module.
  const bulkModuleNeighbors = opts.codeReader.getBulkNeighbors(moduleIds, 'both', 50);
  ...
  for (const module of modules) {
    ...
    createAutoNote(opts, result, {
      ...
      autoContent: renderModuleAutoGenerated(module, degree, bulkModuleNeighbors.get(module.id) ?? []),
...
function autoGenerateRouteNotes(opts: GenerateOptions, result: GenerateResult): void {
  ...
  // R47 (H2): bulk-fetch neighbors for ALL routes in 3 queries (was N×1).
  const routeIds = routes.map((r) => r.id);
  const bulkRouteNeighbors = opts.codeReader.getBulkNeighbors(routeIds, 'out', 30);
  ...
  for (const route of routes) {
    ...
    createAutoNote(opts, result, {
      ...
      autoContent: renderRouteAutoGenerated(route, bulkRouteNeighbors.get(route.id) ?? []),
...
function renderNoteForVault(node: HumanNode, codeReader?: CodeGraphReader, preloadedCodeNodes?: Map<number, CodeNode>): string {
  const fm = buildFrontmatter(node);
  const autoContent = renderAutoGeneratedSection(node, codeReader, preloadedCodeNodes);
  const body = mergeSections(`# ${node.title}`, autoContent, node.body_markdown);
  return serializeNote(fm, body);
}

function renderAutoGeneratedSection(node: HumanNode, codeReader?: CodeGraphReader, preloadedCodeNodes?: Map<number, CodeNode>): string {
  const lines: string[] = [];
  ...
  if (node.cbm_node_ids.length > 0) {
    ...
    lines.push('');
    // R40 (M4): use the page-level preloaded code nodes map if provided
    // (syncHumanNodesToVault bulk-fetches all cbm_node_ids for a page in ONE
    // query, eliminating the N+1 pattern of 1 getNodesByIds call per note).
    // Falls back to a per-node getNodesByIds call when no preload is provided
    // (e.g., when renderNoteForVault is called from outside syncHumanNodesToVault).
    if (codeReader) {
      const nodesMap = preloadedCodeNodes ?? codeReader.getNodesByIds(node.cbm_node_ids);
...
function renderModuleAutoGenerated(
  ...
  degree: number,
  neighbors: { edge: { id: number; project: string; source_id: number; target_id: number; type: string; properties_json: string }; node: { id: number; label: string; name: string; file_path: string | null; start_line: number | null; end_line: number | null; properties_json: string } }[],
): string {
  const lines: string[] = [];
