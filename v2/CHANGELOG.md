...
- **`prepare_edit_context` H1 (BUG)**: `getBulkNotesByCbmNodeIds` was called WITHOUT a limit argument, defaulting to 1. The flagship tool silently under-reported linked notes — agents saw "1 known bug" when 10 bugs were linked. Risk scores were distorted. Fixed: pass `limit=200` (matching `get_module_context`).

- **`generator.ts` H2 (PERF)**: `autoGenerateModuleNotes` and `autoGenerateRouteNotes` called `getNeighbors` per module/route — 200+ queries for 100 modules + 200 routes. Fixed: use `getBulkNeighbors` (3 queries total for all modules, 3 for all routes). Refactored `renderModuleAutoGenerated`/`renderRouteAutoGenerated` to accept pre-fetched neighbors arrays.

- **`routeDashboard` H3 (PERF)**: called `countNodes`, `countEdges`, `countNodesByLabel` — 3 uncached SQLite scans that duplicated work already done inside the SWR-cached `getGraphStatus` call on the same line. Fixed: reuse `graphStatus.total_nodes`/`total_edges`/`nodes_by_label`. Saves 3 queries per dashboard refresh.

### MEDIUM fixes

- **`ControlTab` M1 (LEAK/PERF)**: still used `mountedRef` pattern instead of `AbortController` (R45 fixed this in `useProjects` but not here). The 10s `setInterval` kept firing on slow backends, piling up 3-request batches. Fixed: pass `AbortSignal` to each API call, abort previous batch before starting new one. Also cleaned up the kill-refresh `setTimeout` on unmount (L3).

- **`hotspots` report M3 (BUG)**: `getBulkNotesByCbmNodeIds` called with `limit=1`, so `notes_count` in the exported report was capped at 1 even when 10 notes were linked. Fixed: `limit=200`.

- **`parseNote` M4/F9 (EDGE-CASE)**: the regex `/^---\n([\s\S]*?)\n?---\n/` could match a `---` inside a quoted YAML string value (e.g. `title: "a --- b"`), truncating the frontmatter and corrupting the note on re-export. Fixed: defensive check — if the body starts with a stray `---\n` after parsing, the regex matched too early; treat the whole content as body to preserve the original note.

### LOW fixes (defensive hardening)

- **`swr-cache` L1 (RACE)**: `scheduleBackgroundRefresh`'s `setTimeout(0)` was never cancelled. `invalidate` could run while a refresh was pending, and the refresh would re-insert the just-invalidated value. Fixed: track pending timers per key, cancel on `invalidate`/`clear`.

- **`syncCbmLinks` L2 (LATENT)**: the `DELETE FROM human_node_cbm_links` was outside the inner transaction — safe only because both callers wrapped it in an outer transaction. If a future caller invoked `syncCbmLinks` standalone, a failure in the INSERTs would leave the junction table emptied. Fixed: move DELETE inside the transaction so `syncCbmLinks` is self-contained atomic.

- **`ControlTab.handleKill` L3 (LEAK)**: `setTimeout(refresh, 500)` after a kill was never cleaned up on unmount. Fixed: track in `killTimerRef`, clear on cleanup.

### Test coverage
- 6 new tests in `tests/round47-fixes.test.ts`: H1 getBulkNotesByCbmNodeIds returns multiple notes, M4 parseNote `---` in quoted YAML, L1 swr-cache invalidate cancels refresh, L1 clear() cancels all timers, M4 well-formed note not corrupted, M4 empty frontmatter.
