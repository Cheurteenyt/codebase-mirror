...
This is the maintainer workflow for external AI audits and reset-prone
...
[GLM GitHub operations](GLM_GITHUB_OPERATIONS.md). GLM receives only a repository-scoped SSH deploy
...
[Restricted-environment Git transport](RESTRICTED_ENVIRONMENT_GIT_TRANSPORT.md).
Never commit those recovery files.
...
[Restricted-environment Git transport](RESTRICTED_ENVIRONMENT_GIT_TRANSPORT.md);
clone and fetch may remain HTTPS.
