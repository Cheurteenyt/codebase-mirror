...
> **Authoritative snapshot of the current product state.** Updated R169B-STEP7 (2026-07-14) — recovery completion, real fault evidence, concurrency, documentation and performance closure.
> R169A landed the generation-store foundation (path helpers, manifest V1 types, resolver, atomic JSON writer) and is merged. R169B-STEP2 added the publisher / CAS / GC primitives. R169B-STEP3 closed the 22 findings from the GPT 5.6 Pass 1 audit. R169B-STEP4 closed the 19 findings from the GPT 5.6 Pass 2 audit (immutability via copy/reflink, GC Model A). R169B-STEP5 closed the 16 findings from the GPT 5.6 Pass 3 audit (token state machine, CAS EEXIST, metadata no-clobber, catalog rebuild, final DB mode 0600). R169B-STEP6 addressed the 17 findings from the GPT 5.6 Pass 4 audit (structured PublicationMutationState, reservation token WeakMap, identity-safe cleanup with FinalCleanupResult, ownership verification). R169B-STEP7 (this revision) addresses the 17 findings from the GPT 5.6 Pass 5 audit — cleanup durable certification, noMutation fix (metadata.preexisted not counted), copy error cleanup wired, reservation lifecycle try/catch. All seven remain FOUNDATION / INACTIVE — the indexer and readers still use the legacy `<project>.db` path. `DATA-CARRY-01` (P1) remains open until R169E (after crash matrix + concurrency + performance + activation).
